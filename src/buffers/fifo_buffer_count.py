...
class FIFOBufferCount:
    ...
    It also counts the times each example gets sampled
    """
    def __init__(self, buffer_size, alpha_ema=1.0):
        ...
        self.sampled_count = [] # Buffer for the number of times each sample has been sampled
    ...
    def add(self, batch_x, batch_features, batch_count):
        ...
        self.sampled_count.extend(batch_count)

        if len(self.buffer) > self.buffer_size:
            # Remove oldest samples
            self.buffer = self.buffer[-self.buffer_size:]
            self.buffer_features = self.buffer_features[-self.buffer_size:]
            self.sampled_count = self.sampled_count[-self.buffer_size:]
    ...
    def sample(self, batch_size):
        ...
        batch_count = torch.stack([count for count in counts])

        return batch_x, batch_features, batch_count, indices
