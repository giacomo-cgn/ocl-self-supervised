...
# Attention! CaSSLe is task boundaries-aware
class CaSSLe():
    ...
    def train_experience(self, 
        ...
        self.frozen_encoder = copy.deepcopy(self.model.get_encoder())
        self.frozen_projector = copy.deepcopy(self.model.get_projector())
        ...
        for epoch in range(self.train_epochs):
            for mb_idx, mbatch in tqdm(enumerate(data_loader)):
                ...
                for k in range(self.mb_passes):
                    ...
                    # Frozen model pass
                    with torch.no_grad():
                        frozen_e1 = self.frozen_encoder(x1)
                        frozen_e2 = self.frozen_encoder(x2)
                        frozen_z1 = self.frozen_projector(e1)
                        frozen_z2 = self.frozen_projector(e2)
                    ...
                    if self.align_after_proj:
                        ...
                        loss_align = 0.5*self.align_criterion(aligned_features_1, frozen_z1) + 0.5*self.align_criterion(aligned_features_2, frozen_z2)
                    else:
                        ...
                        loss_align = 0.5*self.align_criterion(aligned_features_1, frozen_e1) + 0.5*self.align_criterion(aligned_features_2, frozen_e2)
